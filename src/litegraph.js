...
// *************************************************************
    ...
    var LiteGraph = global.LiteGraph = {
        ...
        getNodeTypesInCategory: function (category) {
            var r = [];
            for (var i in this.registered_node_types) {
                const ri = this.registered_node_types[i];
                if (category === "") {
                    if (!ri.category)
                        r.push(ri);
                } else if (category === ri.category) {
                    r.push(ri);
                }
            }
            return r;
        ...
        cloneObject: function (obj, target) {
            ...
            var r = _.cloneDeep(obj); // JSON.parse(JSON.stringify(obj));
            if (!target)
                return r;
        ...
        isValidConnection: function (type_a, type_b) {
            return !type_a ||  //generic output
                !type_b || //generic input
                type_a === type_a || //same type (is valid for triggers)
                (type_a !== LiteGraph.EVENT && type_b !== LiteGraph.EVENT && type_a.toLowerCase() === type_b.toLowerCase());
...
//This is more internal, it computes the order and returns it
    LGraph.prototype.computeExecutionOrder = function (only_onExecute) {
        ...
        if (LiteGraph.debug && L.length !== this._nodes.length)
    ...
    LGraph.prototype.add = function (node, skip_compute_order) {
        var nid = node.id;
        if (nid !== undefined) {
            //test for already added
            if (((nid !== -1) && this._nodes_by_id[nid]))
        ...
        if (nid === undefined || nid === -1)
            nid = node.id = ++this.last_node_id;
        else if (this.last_node_id < nid)
            this.last_node_id = nid;

        node.graph = this;

        this._nodes.push(node);
        this._nodes_by_id[nid] = node;
    ...
    LGraph.prototype.findNodesByClass = function (classObject) {
        var r = [];
        for (var i = 0, l = this._nodes.length; i < l; ++i) {
            var ni = this._nodes[i];
            if (ni.constructor === classObject)
                r.push(ni);
        }
        return r;
    };
...
// Execution *************************
    ...
    LGraphNode.prototype.isInputConnected = function (slot) {
        if (!this.inputs)
            return false;
        return (slot < this.inputs.length && this.inputs[slot].link);
    };
...
//connections
    ...
    LGraphNode.prototype.connect = function (slot, target_node, target_slot) {
        ...
        if (target_node.inputs[target_slot].link)
...
//*********************************************************************************
    ...
    global.LGraphCanvas = LiteGraph.LGraphCanvas = function LGraphCanvas(canvas, graph, options) {
        ...
        this.title_text_font = "bold 14px";
        this.inner_text_font = "normal 12px";
...
//this is useful if you plant to render 3D objects inside your nodes
    ...
    LGraphCanvas.prototype.isOverNodeBox = function (node, canvasx, canvasy) {
        ...
        return !!isInsideRectangle(canvasx, canvasy, node.pos[0] + 2, node.pos[1] + 2 - title_height, title_height - 4, title_height - 4);
...
//called if the graph doesnt have a default drop item behaviour
    ...
    LGraphCanvas.prototype.drawFrontCanvas = function () {
        ...
        if (this.graph) {
            ...
            //current connection
            var canvasMouse = this.canvas_mouse;
            if (this.connecting_pos && canvasMouse) {
                ...
                this.renderLink(ctx, this.connecting_pos, [canvasMouse[0], canvasMouse[1]], link_color);
        ...
        if (!node.flags.collapsed) {
            ...
            if (node.inputs)
                for (var i = 0; i < node.inputs.length; i++) {
                    ...
                    ctx.fillStyle = slot.link ? "#7F7" : "#AAA";
...
//OPTIMIZE THIS: precatch connections position instead of recomputing them every time
    ...
    LGraphCanvas.prototype.resize = function (width, height) {
        ...
        if (parseInt(this.canvas.width) === width && parseInt(this.canvas.height) === height)
            return;
    ...
    LGraphCanvas.onMenuAdd = function (node, e, prev_menu, canvas, first_event) {
        ...
        return false; //must return false to capture the event or something
    };
    ...
    LGraphCanvas.prototype.showEditPropertyValue = function (node, property, options) {
        ...
        if (type === "string" || type === "number")
            ...
            input_html = "<select class='value' autofocus>";
...
//	the corresponding hex triplet.
    function num2hex(triplet) {
        ...
        for (var i = 0; i < 3; i++) {
            var ti = triplet[i];
            hex += hex_alphabets.charAt(ti / 16) + hex_alphabets.charAt(ti % 16);
        }
        return (hex);
    }
    ...
    LiteGraph.closeAllContextMenus = function (ref_window) {
        ...
        const elements = ref_window.document.querySelectorAll(".graphcontextmenu");
        elements.forEach(e => {
            $(e).fadeOut('slow', ()=> $(e).remove());
        });
